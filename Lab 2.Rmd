...
* **The first histogram has a range that is far too large (likely skewed by outliers). While you can tell that majority of delays are 0, most of the bins are too small within the x-axis to truly make out the frequency of actual delays not 0.  I can tell that all the delays are less than 250**  
  
* **The second histogram shows the shape of distribution a bit better.  However, it can be argued that bin range is too narrow and is producng too many bins to focus on/ too much noise.**  
  
* **The bins of the third histogram are too wide and obscure the shape of the distribution altogether. **  
...
```
**It would appear that arrival "delays" have a normal bell-curve distribution, with a center of -2 minutes.  Arrival delays are actually arriving 2 minutes earlier than expected.  Mean is being used to calculate average here as the data is normally distributed**  
...
sfo_feb_flights %>%
  ...
  summarise(
    ...
    iqr_dd = IQR(arr_delay), 
    sd_dd = round(sd(arr_delay),3),
    n_flights = n())
```
  
**carrier UA -  widest IQR and standard deviation**
...
nycflights %>%
  group_by(month) %>%
  summarise(mean_dd = round(mean(dep_delay),3)) %>%
...
**mean**
* **pro: useful statistic in a lot of further statistical testing**
* **pro: contains/considers every data point**
* **con: not a robust statistic so if there are a few extreme outlier departure delays they will skew the average considerably**

**median**
* **pro:  robust - if there are outlier delays, median will still capture measure of center**
* **con: cannot be used in further mathematical calculations, not very useful for statistical tesing**
...
8.  Make a scatterplot of `avg_speed` vs. `distance`. Describe the relationship
    ...
    **Hint:** Use `geom_point()`.
    ```{r scatterplot-distance-avg_speed}
ggplot(nycflights, aes(x=distance, y=avg_speed)) + geom_point()
```
**We can see a modest correlation between distance and avg speed that seems to level off around 1000 miles**
...
**Create a new boolean column for whether a flight arrived on time.**
```{r arr-type}
dl_aa_ua <- dl_aa_ua %>%
  mutate(arr_type = ifelse(arr_delay < 5, "on time", "delayed"))
```

**Plot a scatterplot to see visually the flights that arrived on time vs were delayed.**
```{r plot-delays-by-arr_type, echo=FALSE}
ggplot(data = dl_aa_ua, aes(x = dep_delay, y = arr_delay, color = arr_type)) +
  geom_point()
```
Filter dataframe to look at only flights where departure was delayed, but arrival was on time    
  
Take the max value of dep_delay variable to see where greatest delay took place but plane arrived on time  = 63 minutes.
On average, of flights that arrived on time and had a delayed departure (> 5minutes), the Interquartile Range for the departure delay was between 7 to 16 minutes.
```{r filter ot-arr d-dep and calc max dep}
arr_on_time <- dl_aa_ua %>%
  filter(arr_type == "on time", dep_type == "delayed")

summary(arr_on_time$dep_delay)
```

```{r ot-arr-rate of dl_aa_ua}
dl_aa_ua %>%
  group_by(carrier) %>%
  summarise(ot_arr_rate = round(sum(arr_type == "on time") / n(),2)) %>%
  arrange(desc(ot_arr_rate))
```
Just for fun, look at departure delay rate and arrival on time rate for carriers Delta, American Airlines, United Airlines
```{r del-dep-rate of dl_aa_ua}
dl_aa_ua %>%
  group_by(carrier) %>%
  summarise(del_dep_rate = round(sum(dep_type == "delayed") / n(),2)) %>%
