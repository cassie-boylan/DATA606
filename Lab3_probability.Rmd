...
**Streak length of 1 indicates that only hit was made before being followed by a miss.  Each streak ends when the ball misses the hoop.  A streak length of 0 indicates that ball missed the hoop without any preceding hits**
...
**In my simulation (below), heads came up 15 times, tails came up 85 times.**

```{r run-unfair-coin-simulation}
coin_outcomes <- c("heads", "tails")
sim_unfair_coin <- sample(coin_outcomes, size = 100, replace = TRUE, 
                          prob = c(0.2, 0.8))
```
...
**You need to add a probability argument that specifies the probability of hit outcome and miss outcome.**
```{r}
set.seed(92199)
